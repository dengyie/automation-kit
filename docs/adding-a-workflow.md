...
Built-in workflow entries must have matching discovery metadata before they can
be emitted by `examples --json`.
...
- Selenium: `open`, `click`, `type_text`, `wait_for_element`
- Appium: `launch_app`, `tap`, `type_text`, `wait_for_element`
...
`docs/report-schema-v1.json` and can be printed from installed environments
with `automation-runner report-schema --version 1`.
