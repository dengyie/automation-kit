...
| 仓库 | 平台角色 | 拥有内容 | 禁止内容 | 当前基线 |
| --- | --- | --- | --- | --- |
| `automation-kit` | 执行内核与公共 SDK | 核心模型、能力契约、runner、通用 adapter、报告 | 业务规则、Slidex 或厂商硬依赖 | `2e5eb7e` |
| `slidex` | 可选视觉能力 provider | slider、OCR、图像识别、人工兜底、视觉 telemetry | 点评/大麦流程、反向要求核心依赖 Slidex | `5cc1ab9` |
| `automation-app-damai` | Web/业务应用 | 大麦配置、流程、业务验收、能力请求组装 | 通用执行内核、视觉算法 | `3ccd788` |
| `automation-app-dianping` | Android/业务应用 | 点评配置、流程、业务验收、能力请求组装 | 通用执行内核、OCR 实现 | `3a1c94e` |
| `automation-plugin-ocr` | 归档仓库 | 仅保留迁移说明 | 任何新功能 | `1b44c77` |
...
- 同步入口不得隐式创建或嵌套事件循环；异步 provider 必须通过异步入口调用。
...
异步入口调用同步 provider 时，使用 Python 3.8 可用的线程 executor，不能在事件循环
线程中直接执行阻塞操作。同步入口调用异步 provider 时立即报调用模式错误，不隐式
创建事件循环。provider 抛出的原始异常文本不得进入结果；失败结果只保留错误类型和
固定安全摘要，详细诊断只能进入经过脱敏与访问控制的日志系统。
...
| 调用模式错误 | 同步入口调用异步 provider | 明确要求异步入口 |
...
- app 声明兼容的 automation-kit 版本范围，不长期依赖未发布的 sibling path。
...
配置完整的 CI 还必须执行仓库原有 coverage 门槛。当前本机缺少 `poetry` 和
`pytest-cov`，本地 smoke 使用 `-o addopts=''`，不能据此宣称 coverage 达标。

### 10.3 2026-07-18 验证基线

| 范围 | 结果 |
| --- | --- |
| automation-kit 全量离线测试 | `322 passed` |
| Damai + automation-kit + Slidex 跨仓测试 | `17 passed` |
| Dianping + automation-kit + Slidex 跨仓测试 | `14 passed` |
| Slidex + automation-kit 全量测试 | `257 passed` |
| 归档 OCR 基本测试 | `2 passed` |
| 五仓 `git diff --check` | 通过 |

当前机器没有 Python 3.8 解释器，automation-kit 与 app 的 3.8 兼容完成了语法和依赖
静态复核，但仍必须由 CI 的 Python 3.8 job 提供运行时证据。
...
- Slidex 已有统一视觉 request/result、OCR、slider、provider manifest、人工兜底和
  automation-kit action/event/artifact 映射。
- Slidex 已通过 `SlidexVisualCapability` 实现通用视觉能力 provider。
- 大麦和点评已有通用 capability 入口、旧 Slidex 兼容 helper 和跨仓纵向测试。
- OCR 独立插件已归档。
...
4. 大麦和点评新增只依赖 automation-kit 的能力请求/执行入口；保留旧 Slidex helper
   作为兼容面，不再把它作为新应用模板。
5. 增加三仓纵向契约测试，证明 app 可以通过通用 executor 调用 fake/Slidex provider。
6. 经独立审查补强异常脱敏、异步非阻塞执行和双入口协议错误一致性。
...
交付：发布 automation-kit 契约版本；app 和 Slidex 使用版本范围；CI 建立四仓契约矩阵。
验收：不使用 sibling path 也能完成安装与离线测试，兼容失败会阻止发布。

#### 阶段 4：provider 运行治理

交付：provider 优先级、健康状态、超时、重试策略和可选熔断；策略仍由内核控制，
provider 不自行吞掉平台级超时。验收：多 provider 选择和故障转移完全由 deterministic
测试证明。
...
- 修改 `slidex/integrations/automation_kit.py`：增加 `SlidexVisualCapability`。
- 修改 `tests/test_automation_kit_integration.py`：覆盖通用能力请求、结果和资源所有权。
...
3. `git diff --check` 通过。
4. 仓库默认测试通过；涉及公共契约时跨仓兼容测试通过。
5. 公共接口、架构、状态或验收命令变化时同步更新本文。
6. 不重新引入开发日志、重复架构文档、隐藏项目状态或已完成计划目录。
